...
async function getGithubStats() {
  try {
    const userRes = await fetch('https://api.github.com/users/polaris3535', { next: { revalidate: 3600 } });
    const userData = await userRes.json();

    const reposRes = await fetch('https://api.github.com/users/polaris3535/repos?per_page=100', { next: { revalidate: 3600 } });
    ...
    return { repoCount: 0, followers: 0, stars: 0 };
  }
}
...
export default async function Home() {
  // İki API'yi de burada çağırıyoruz
  const stats = await getGithubStats();
  const publishedGames = await getGames(); // Artık veriler API'den geliyor!
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500/30 font-sans">
      ...
      <section className="flex flex-col items-center justify-center pt-16 pb-24 md:pt-20 md:pb-32 px-6 text-center">
        ...
        <div className="flex gap-4 mb-10">
          <div className="flex items-center gap-2 bg-zinc-900/50 border border-zinc-800 px-4 py-2 rounded-lg text-sm text-gray-300">
            <BookOpen size={16} className="text-blue-400" />
            <span className="font-bold text-white">{stats.repoCount}</span>
            <span>Repo</span>
          </div>
          <div className="flex items-center gap-2 bg-zinc-900/50 border border-zinc-800 px-4 py-2 rounded-lg text-sm text-gray-300">
      ...
      <section id="projects" className="max-w-6xl mx-auto px-4 md:px-6 py-16 md:py-20">
        ...
        {/* EĞER API'DEN VERİ GELMEZSE BOŞ EKRAN GÖSTERMESİN DİYE KONTROL */}
        {publishedGames.length === 0 ? (
          <div className="text-center text-gray-500 py-10">Oyun verileri yükleniyor veya API'ye ulaşılamadı...</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {publishedGames.map((game: any, i: number) => {
              ...
              return (
                <div key={i} className="group flex flex-col bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all duration-500 hover:shadow-[0_0_30px_-10px_rgba(59,130,246,0.2)]">
                  <div className="relative w-full aspect-video overflow-hidden bg-zinc-950">
                    {/* Image src API'den düzgün gelmeli. Eğer public klasöründeyse sorun yok */}
                    <Image
                      src={game.image}
                      alt={game.title}
                      fill
                  ...
                  <div className="p-4 md:p-6 flex flex-col flex-grow">
                    <div>
                      ...
                      <p className="text-gray-400 text-xs md:text-sm leading-relaxed mb-5 md:mb-6 line-clamp-none md:line-clamp-3">
                        {game.description}
                      </p>
                    </div>
                    ...
                    <div className="mt-auto grid gap-2 md:gap-3">
                      {!isPublished ? (
                        ...
                        <>
                          {game.androidLink && (
                            <a href={game.androidLink} target="_blank" className="flex items-center justify-center gap-3 w-full bg-[#1a1a1a] hover:bg-[#252525] text-white py-2.5 md:py-3 rounded-xl border border-zinc-800 hover:border-green-600/50 transition-all duration-300 group/btn">
                              ...
                              <div className="text-left leading-none flex flex-col justify-center">
                                <div className="text-[9px] md:text-[10px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">Get it on</div>
                                <div className="text-sm md:text-base font-bold font-sans">Play Store</div>
                              </div>
                            </a>
                          )}
                          {game.iosLink && (
                            <a href={game.iosLink} target="_blank" className="flex items-center justify-center gap-3 w-full bg-[#1a1a1a] hover:bg-[#252525] text-white py-2.5 md:py-3 rounded-xl border border-zinc-800 hover:border-gray-400/50 transition-all duration-300 group/btn">
