...
const publishedGames = [
  ...
  },
  {
    title: "Gizli Proje",
    image: "/proje-x.png",
    description: "Henüz geliştirme aşamasında olan yeni bir RPG deneyimi.",
    tech: ["Unity", "C#"],
    androidLink: "", // Boş bırakırsan "Yakında" yazar
    iosLink: ""      // Boş bırakırsan "Yakında" yazar
  }
];
            ...
            return (
              <div key={i} className="group flex flex-col bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all duration-500 hover:shadow-[0_0_30px_-10px_rgba(59,130,246,0.2)]">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  {/* Butonlar Alanı - Güncellenmiş ve Hizalı */}
                  <div className="mt-auto grid gap-3">
                    
                    {!isPublished ? (
                      // ... Yakında Yayınlanacak butonu ...
                      <div className="flex items-center justify-center gap-2 w-full bg-zinc-800/50 text-gray-400 py-3 rounded-xl border border-zinc-700/50 cursor-not-allowed">
                        <svg className="w-5 h-5 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span className="text-sm font-medium">Yakında Yayınlanacak</span>
                      </div>
                    ) : (
                      <>
                        {/* Play Store Button (Doğru Logo ve Hizalama) */}
                        {game.androidLink && (
                          <a 
                            href={game.androidLink} 
                            target="_blank" 
                            className="flex items-center justify-center gap-3 w-full bg-[#1a1a1a] hover:bg-[#252525] text-white py-3 rounded-xl border border-zinc-800 hover:border-green-600/50 transition-all duration-300 group/btn"
                          >
                            <svg className="w-7 h-7 fill-current text-green-500 group-hover/btn:text-green-400 flex-shrink-0" viewBox="0 0 512 512">
<path d="M325.3 234.3L104.6 13l280.8 161.2-60.1 60.1zM47 0C34 6.8 25.3 19.2 25.3 35.3v441.3c0 16.1 8.7 28.5 21.7 35.3l256.6-256L47 0zm425.2 225.6l-58.9-34.1-65.7 64.5 65.7 64.5 60.1-34.1c18-14.3 18-46.5-1.2-60.8zM104.6 499l280.8-161.2-60.1-60.1L104.6 499z" />                            </svg>
                            <div className="text-left leading-none flex flex-col justify-center">
                              <div className="text-[10px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">Download on the</div>
                              <div className="text-base font-bold font-sans">Play Store</div>
                            </div>
                          </a>
                        )}

                        {/* App Store Button (Doğru Logo ve Hizalama) */}
                        {game.iosLink && (
                          <a 
                            href={game.iosLink} 
                            target="_blank" 
                            className="flex items-center justify-center gap-3 w-full bg-[#1a1a1a] hover:bg-[#252525] text-white py-3 rounded-xl border border-zinc-800 hover:border-gray-400/50 transition-all duration-300 group/btn"
                          >
                            <svg className="w-7 h-7 fill-current text-gray-300 group-hover/btn:text-white flex-shrink-0" viewBox="0 0 24 24">
